...
vi.mock('./Modal', () => ({ children }) => <div data-testid="modal">{children}</div>);
vi.mock('./NftCard', () => () => <div data-testid="nft-card">NftCard</div>);
vi.mock('./TokenCard', () => () => <div data-testid="token-card">TokenCard</div>);
vi.mock('./HistoryCard', () => () => <div data-testid="history-card">HistoryCard</div>);
...
describe('ViewWallet', () => {
  ...
  it('renders without crashing', async () => {
    render(
      <MemoryRouter>
        <ViewWallet wallet="0x123" seedPhrase="test seed" selectedChain={TEST_CHAIN_KEY} />
      </MemoryRouter>
    );
  ...
  it('switches to NFT tab and displays NFTs', async () => {
    ...
    const nftTab = screen.getByRole('tabpanel', { name: 'NFTs' });
...
/*
  it('switches to History tab and displays transactions', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByText('Aug 8, 2024')).toHaveLength(2);
    });
  });

  it('opens modal when clicking on a token', async () => {
    render(
      <MemoryRouter>
        <ViewWallet wallet="0x123" seedPhrase="test seed" selectedChain={TEST_CHAIN_KEY} />
      </MemoryRouter>
  ...
  it('opens modal when clicking on an NFT', async () => {
    render(
      <MemoryRouter>
        <ViewWallet wallet="0x123" seedPhrase="test seed" selectedChain={TEST_CHAIN_KEY} />
      </MemoryRouter>
    ...
    await waitFor(() => {
      fireEvent.click(screen.getByText('Azuki #70'));
    });
    await waitFor(() => {
      expect(screen.getByTestId('modal')).toBeInTheDocument();
  ...
  it('opens modal when clicking on a history item', async () => {
    render(
      <MemoryRouter>
        <ViewWallet wallet="0x123" seedPhrase="test seed" selectedChain={TEST_CHAIN_KEY} />
      </MemoryRouter>
    ...
    fireEvent.click(screen.getByRole('tab', { name: /activity/i }));
    await waitFor(() => {
      fireEvent.click(screen.getAllByText('Azuki')[0]);
    });
    await waitFor(() => {
      expect(screen.getByTestId('modal')).toBeInTheDocument();
  ...
  it('displays loading state when fetching data', () => {
    ...
    expect(screen.getAllByTestId('skeleton')).toHaveLength(3); // Assuming 3 skeleton loaders are shown
  });

  it('displays no tokens message when no tokens are available', () => {
    vi.mocked(useFetchTokensAndNfts).mockReturnValue({
      tokens: [],
      nfts: [],
      historys: [],
  ...
  it('displays no NFTs message when no NFTs are available', async () => {
    vi.mocked(useFetchTokensAndNfts).mockReturnValue({
      tokens: mockTokens,
      nfts: [],
      historys: [],
  ...
  it('displays no history message when no history is available', async () => {
    vi.mocked(useFetchTokensAndNfts).mockReturnValue({
      tokens: mockTokens,
      nfts: mockNfts,
      historys: [],
